...
//    }
    ...
    public ResponseEntity<Map<String, Object>> getArticle( // 반환 타입을 Map<String, Object>로 변경
                                                           ...
                                                           CurrentUser currentUser) { // HttpServletRequest를 통해 요청 속성 접근

        log.info("게시글 단건 조회 요청 - ID: {}, 요청자 ID: {}", articleId, currentUser.getId());
        try {
            ArticleResponse article = articleService.findArticleById(articleId, currentUser.getId());

            log.info("게시글 단건 조회 요청 완료");

            return ApiResponseUtil.success("게시글 조회 성공", article);


        } catch (IllegalArgumentException e) {
            log.warn("게시글 단건 조회 실패 - ID: {}, 사유: {}", articleId, e.getMessage());
            // 예외 처리는 이미 Util을 사용하고 있으므로 그대로 둡니다.
            return ApiResponseUtil.badRequest(e.getMessage());
        } catch (Exception e) {
            log.error("게시글 단건 조회 중 서버 오류 발생 - ID: {}", articleId, e);
    ...
    public ResponseEntity<Map<String, Object>> getArticles( // 반환 타입을 Map<String, Object>로 변경
        ...
        log.info("게시글 목록 조회 요청 - 조건: {}, 페이징: {}, 요청자 ID: {}", condition, pageable, currentUser.getId());

        try {
            Page<ArticleResponse> articlesPage = articleService.searchArticles(condition, pageable, currentUser.getId());
            log.info("게시글 목록 조회 완료");

            // ▼▼▼ [수정] 이 부분도 ApiResponseUtil로 변경합니다 ▼▼▼
            return ApiResponseUtil.success("게시글 목록 조회 성공", articlesPage);
            // ▲▲▲ [수정] 끝 ▲▲▲

        } catch (Exception e) {
            log.error("게시글 목록 조회 중 서버 오류 발생 - 조건: {}, 페이징: {}", condition, pageable, e);
            return ApiResponseUtil.internalServerError("서버 내부 오류가 발생했습니다.");
        }
    }
